...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val aggreDF = joinDf.groupBy("rectangle").count().sort("rectangle").coalesce(1)

    aggreDF.createOrReplaceTempView("aggResult")

    // YOU NEED TO CHANGE THIS PART

    return aggreDF // YOU NEED TO CHANGE THIS PART
  }
